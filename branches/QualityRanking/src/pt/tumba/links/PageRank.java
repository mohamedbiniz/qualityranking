...
public class PageRank {
   ...
   public double getDampening() {
	...
	public void computePagerank(int iter) {
	  ...
	  while((iter--)>0) {     
		  Map<Integer, Double> newScore = new HashMap<Integer, Double>();
		  for (int j=0;j<n;j++) { 
			Map<Integer, Double> inlinks = graph.inLinks(new Integer(j));
			Iterator<Integer> it = inlinks.keySet().iterator();
			...
			while (it.hasNext()) {
				...
				if(weight!=null && weight.doubleValue()>0) {
					int numLinks = 0;
					Map<Integer, Double> outlinks = graph.outLinks(link);
					Iterator<Integer> it2 = inlinks.keySet().iterator();
					while (it.hasNext()) {

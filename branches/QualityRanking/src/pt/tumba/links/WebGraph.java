...
public class WebGraph {
	...
	public Integer URLToIdentifyer ( String URL ) {
		...
		Map<String, Integer> map = (Map<String, Integer>)(URLToIdentifyer.get(host));
	...
	private Double addLink ( Integer fromLink, Integer toLink, Double weight ) {
		Double aux;
		Map<Integer, Double> map1 = (Map<Integer, Double>)(InLinks.get(toLink));
		Map<Integer, Double> map2 = (Map<Integer, Double>)(OutLinks.get(fromLink));
	...
	public Map<Integer, Double> inLinks ( Integer link ) {
		...
		Map<Integer, Double> aux = (Map<Integer, Double>)(InLinks.get(link));
	...
	public Map<Integer, Double> outLinks ( Integer link ) {
		if(link==null) return new HashMap<Integer, Double>();
		Map<Integer, Double> aux = (Map<Integer, Double>)(OutLinks.get(link));
		return (aux == null) ? new HashMap<Integer, Double>() : aux;
	}
	...
	public void transformUnidirectional () {
		...
		while (it.hasNext()) {
			...
			Map<Integer, Double> auxMap = (Map<Integer, Double>)(OutLinks.get(link1));
			...
			if(it2.hasNext()) {
				...
				while (it2.hasNext()) {
					...
					if(URL1.equals(URL2)) {
						...
						auxMap = (Map<Integer, Double>)(InLinks.get(link2));
						auxMap.remove(link1);
	...
	public void removeStopLinks(String stopURLs[]) {
		// RSN determinar o tipo disso HashMap<String, Object> aux 

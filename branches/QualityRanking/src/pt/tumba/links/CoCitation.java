...
public class CoCitation {
	 ...
	 */
	public Map<Integer, Double> cocitation(String link) {
	 ...
	 * @return A Map with the CoCitation score between the given link and all other
         ...
         *         and values correspond to the CoCitation score
	 */
	private Map<Integer, Double> cocitation(Integer id) {
		if(id.intValue()!=0) {
			if ((scores.get(id)).get(new Integer(0)).doubleValue()<0) computeCocitation(id);
		} else {
			if ((scores.get(new Integer(1)).get(new Integer(0))).doubleValue()<0) computeCocitation(id);
		}
	...
	private Double computeCocitation(Integer id1, Integer id2) {
		...
		Map<Integer, Double> map3 = (Map<Integer, Double>)(scores.get(id1));
		map3.put(id2,score);
		scores.put(id1,map3);
		return score;
	}

...
public class Amsler {
	 ...
	 */
	public Map<Integer, Double> amsler(String link) {
	 ...
	 * @return A Map with the Amsler score between the given link and all other
         ...
         *         and values correspond to the Amsler score
	 */
	private Map<Integer, Double> amsler(Integer id) {
		if(id.intValue()!=0) {
			if (scores.get(id).get(new Integer(0)).doubleValue()<0)
				computeAmsler(id);
		} else {
			if (scores.get(new Integer(1)).get(new Integer(0)).doubleValue()<0)
				computeAmsler(id);
		}
		return (Map<Integer, Double>)(scores.get(id));
	}
	...
	private Double amsler(Integer id1, Integer id2) {
		...
		Double aux = (Double)(((Map<Integer, Double>)(scores.get(id1))).get(id2)); 
	...
	private Double computeAmsler(Integer id1, Integer id2) {
		...
		int score5 = 0;
		Double score;
		Map<Integer, Double> map1 = graph.inLinks(id1);
		Map<Integer, Double> map2 = graph.inLinks(id2);
		Map<Integer, Double> map3 = graph.outLinks(id1);
		Map<Integer, Double> map4 = graph.outLinks(id2);
		...
		Map<Integer, Double> map5 = (Map<Integer, Double>)(scores.get(id1));
		map5.put(id2,score);
		scores.put(id1,map5);
		return score;
	}

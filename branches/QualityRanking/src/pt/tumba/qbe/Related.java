...
public class Related {
	...
	private Map<Long, Map<Long, Double>> cocitations;
	private Map<Long, Map<Long, Double>> couplings;
	private Map<Long, Double> siblings;
	
	private Map<Long, Long> id_to_url;
	private Map<Long, Long> url_to_id;
	
	private Map<Long, Double> url_authority_score;
	private Map<Long, Double> url_hub_score;

	// RSN Determinar o tipo disso: Map<Long, Map<Long, Object>> url_from_urls
	private Map<Long, Map<Long, Object>> url_from_urls;
	// RSN Determinar o tipo disso: Map<Long, Map<Long, Object>> url_to_urls
	private Map<Long, Map<Long, Object>> url_to_urls;
	...
	public Related(Connection conn, Logger logger, int maxLinks) {
		...
		url_id = 0;
		url_to_id = new HashMap<Long, Long>();
		id_to_url = new HashMap<Long, Long>();
		url_hub_score = new HashMap<Long, Double>();
		url_authority_score = new HashMap<Long, Double>();
		url_to_urls = new HashMap<Long, Map<Long, Object>>();
		url_from_urls = new HashMap<Long, Map<Long, Object>>();
		cocitations = new HashMap<Long, Map<Long, Double>>();
		couplings = new HashMap<Long, Map<Long, Double>>();
		siblings = new HashMap<Long, Double>();
		rand = new Random(0);
		this.conn = conn;
	}
    ...
    Long addURL(Long url, boolean cocite) {
        ...
        if ((id = (Long) (url_to_id.get(url))) == null) {
            ...
            url_to_urls.put(id, new HashMap<Long, Object>());
            url_from_urls.put(id, new HashMap<Long, Object>());
            if (cocite) {
     ...
     */
    public void addURL(Long fromURL, Iterator<Long> toURL) {
        Long fromID = addURL(fromURL, false);
        Map<Long, Object> to_urls = url_to_urls.get(fromID);
        Object trash = null;
        while (toURL.hasNext()) {
            Long aux = toURL.next();
            Long toID = addURL(aux, false);
            Map<Long, Object> from_urls = url_from_urls.get(toID);
     ...
     *@return             Description of the Return Value
     */
    List<Long> CoCitation(Long url, boolean return_url) {
        Iterator<Long> it;
        Long id = (Long) (url_to_id.get(url));
        Map<Long, Double> urls = cocitations.get(id);
        if (urls == null) {
        ...
        if (return_url) {
            for (i = 0; i < v.size(); ) {
                if (((Double) (urls.get(v.elementAt(i)))).doubleValue() > 0.0) {
                    v.setElementAt(((Long) id_to_url.get(v.elementAt(i))), i++);
                } else {
                    v.removeElementAt(i);
                }
            }
    ...
    public void computeCocitations() {
        Iterator<Long> it2 = cocitations.keySet().iterator();
        Iterator<Long> it;
        while (it2.hasNext()) {
            Long id = it2.next();
            Map<Long, Object> aux;
            HashMap<Long, Double> urls = new HashMap<Long, Double>();
            it = id_to_url.keySet().iterator();
            while (it.hasNext()) {
                urls.put(it.next(), new Double(0));
            }

            if ((aux = url_to_urls.get(id)) != null) {
                long maxValue = 0;
                Iterator<Long> parent_urls = aux.keySet().iterator();
                while (parent_urls.hasNext()) {
                    Long id2 = parent_urls.next();
                    if ((aux = url_from_urls.get(id2)) != null) {
                        Iterator<Long> child_urls = aux.keySet().iterator();
                        while (child_urls.hasNext()) {
                            Long id3 = child_urls.next();
    ...
    public void computeCouplings() {
        Iterator<Long> it2 = couplings.keySet().iterator();
        Iterator<Long> it;
        while (it2.hasNext()) {
            long maxValue = 0;
            Long id = (Long) (it2.next());
            Map<Long, Object> aux;
            Map<Long, Object> aux2;
            HashMap<Long, Double> urls = new HashMap<Long, Double>();
            it = id_to_url.keySet().iterator();
            while (it.hasNext()) {
                urls.put(it.next(), new Double(0));
            ...
            if ((aux = url_from_urls.get(id)) != null) {
                Iterator<Long> it1 = aux.keySet().iterator();
                while (it1.hasNext()) {
                    Long id2 = it1.next();
                    if ((aux2 = url_to_urls.get(id2)) != null) {
                        Iterator<Long> it3 = aux2.keySet().iterator();
                        while (it3.hasNext()) {
                            Long id3 = it3.next();
                            long intv = ((Double) (urls.get(id3))).longValue() + 1;
                            if (intv > maxValue) {
    ...
    public void computeHITS() {
        ...
        for (int i = 0; i < numIterations && change; i++) {
            ...
            for (int j = 0; j < url_id; j++) {
                ...
                Long id = new Long(j);
                Iterator<Long> to_urls = url_to_urls.get(id).keySet().iterator();
                Iterator<Long> from_urls = url_from_urls.get(id).keySet().iterator();
    ...
    public void computeSiblings() {
        double min = url_id + 1;
        double max = 0;
        Iterator<Long> it2 = siblings.keySet().iterator();
        while (it2.hasNext()) {
            Long id = it2.next();
     ...
     *@return             Description of the Return Value
     */
    List<Long> Coupling(Long url, boolean return_url) {
        Long id = (Long) (url_to_id.get(url));
        Map<Long, Double> urls = couplings.get(id);
        Iterator<Long> it;
        if (urls == null) {
            return new Vector<Long>();
        }
     ...
     *@return     Description of the Return Value
     */
    List<Long> exchangePos(List<Long> v, int p1, int p2) {
        Long tmp = v.get(p1);
        v.set(p1, v.get(p2));
        v.set(p2, tmp);
        return v;
     ...
     */
    public List<Long> getInLinks(String url2) {
		String url = url2;
     ...
     */
    public List<Long> getOutLinks(String url2) {
		String url = url2;
        if (!url.startsWith("http://"))
        	url = "http://"+ url;
        Vector<Long> v1 = new Vector<Long>();
     ...
     */
    public List<Long> getRelated(String url2) {
        loadLinks(url2);
        compute();
        Iterator<Long> it;
        ...
        Long id = (Long) (url_to_id.get(url));
        Map<Long, Double> urls = cocitations.get(id);
        Map<Long, Double> urls2 = couplings.get(id);
        if (urls == null) {
            return new Vector<Long>();
        }
        List<Long> v = new Vector<Long>();
        it = urls.keySet().iterator();
        while (it.hasNext()) {

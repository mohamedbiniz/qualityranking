	...
	class Print_a_class {
		...
		function print_a($array, $iteration = FALSE, $key_bg_color = FALSE) {
			...
			foreach( $array as $key => $value ) {
				...
				$this->output .= '<tr>';
				$this->output .= '<td nowrap="nowrap" align="'.$this->keyalign.'" style="padding:0px 3px 0px 3px;background-color:#'.$key_bg_color.';'.$key_style.';font:bold '.$this->fontsize.' '.$this->fontfamily.';" title="'.$this->_handle_whitespace( $key ) .' - ' . gettype( $key ).'['.$type_title.']">';

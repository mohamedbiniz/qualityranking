		...
		foreach ( $this->getElements() as $ele ) {
			if (!is_object($ele)) {
				...
				if ($ele->isSingleline()) {
					...
					$ret .= "<tr valign='top'><td colspan='2' class='$class'>".$ele->getCaption();
					...
					$ret .= "<tr valign='top'><td class='head'>".$ele->getCaption();
					if ($ele->getDescription() != '') {
						$ret .= '<br /><span style="font-weight: normal;">'.$ele->getDescription().'</span>';
					}
					$ret .= "</td></tr><tr><td class='$class'>".$ele->render()."</td></tr>";
					$count++;
				}
			} else {
				$ret .= $ele->render();
